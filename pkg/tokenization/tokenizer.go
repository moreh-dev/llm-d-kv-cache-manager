...
func (t *HFCachedTokenizer) ApplyChatTemplate(
...
func (t *LocalCachedTokenizer) ApplyChatTemplate(
	modelName string, req *preprocessing.ApplyChatTemplateRequest,
) (string, error) {
	ctx := context.TODO()
...
func (t *LocalCachedTokenizer) Encode(req *preprocessing.EncodeRequest) ([]uint32, []tokenizers.Offset, error) {
	...
	path, ok := t.localTokenizerConfig.ModelTokenizerMap[req.Model]
	if !ok {
		return nil, nil, fmt.Errorf("tokenizer for model %q not found", req.Model)
	}
	req.Model = filepath.Dir(path)
	tokens, offsets, err := t.chatTemplateRenderer.Encode(ctx, req)
	if err != nil {
