...
func (s *KVCacheSuite) TestCacheHit() {
	...
	pods, err := s.indexer.GetPodScores(s.ctx, prompt, nil, defaultModelName, fakePodList)
...
func (s *KVCacheSuite) TestPrefixReduction() {
	...
	pods, err := s.indexer.GetPodScores(s.ctx, fullPrompt, nil, defaultModelName, []string{s.Pod1IP})
	s.Require().NoError(err)
	s.T().Logf("Received pod scores: %+v", pods)
	...
	pods, err = s.indexer.GetPodScores(s.ctx, midPrompt, nil, defaultModelName, []string{s.Pod1IP})
	s.Require().NoError(err)
	...
	pods, err = s.indexer.GetPodScores(s.ctx, shortPrompt, nil, defaultModelName, []string{s.Pod1IP})
	s.Require().NoError(err)
...
func (s *KVCacheSuite) TestPrefixExpansion() {
	...
	// Test 1: short prompt
	pods, err := s.indexer.GetPodScores(s.ctx, shortPrompt, nil, modelName, []string{s.Pod1IP})
	s.Require().NoError(err)
	s.T().Logf("Received pod scores: %+v", pods)
	s.Empty(pods, "expected no pod scores")
	...
	// Test 2: mid prompt
	pods, err = s.indexer.GetPodScores(s.ctx, midPrompt, nil, modelName, []string{s.Pod1IP})
	s.Require().NoError(err)
	...
	// Test 3: full prompt
	pods, err = s.indexer.GetPodScores(s.ctx, fullPrompt, nil, modelName, []string{s.Pod1IP})
	s.Require().NoError(err)
...
func (s *KVCacheSuite) TestLongPrefixExpansion() {
	...
	pods, err = s.indexer.GetPodScores(s.ctx, longPrompt, nil, modelName, []string{s.Pod1IP})
	s.Require().NoError(err)
...
func (s *KVCacheSuite) TestChatCompletionsE2E() {
	...
	pods, err := s.indexer.GetPodScores(s.ctx, flattenedPrompt, nil, "ibm-granite/granite-3.3-8b-instruct", []string{s.Pod1IP})
	s.Require().NoError(err)
