...
func demonstrateValkeyOperations(ctx context.Context, indexer *kvcache.Indexer) error {
	...
	scores, err := indexer.GetPodScores(ctx, prompt, nil, modelName, []string{"demo-pod-1", "demo-pod-2"})
	if err != nil {
	...
	finalScores, err := indexer.GetPodScores(ctx, prompt, nil, modelName, []string{"demo-pod-1", "demo-pod-2"})
	if err != nil {
